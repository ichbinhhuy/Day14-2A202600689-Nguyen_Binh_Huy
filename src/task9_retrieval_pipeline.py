...
def retrieve(
    ...
    # RRF scores are typically < 0.05. If RRF is used (indicated by score < 0.1), use a threshold of 0.01
    ...
    if not final_results or final_results[0]["score"] < actual_threshold:
        ...
        print(f"  ⚠ Hybrid score ({best_score:.3f}) < threshold ({actual_threshold}). Fallback → PageIndex")
